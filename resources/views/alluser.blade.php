   ...
   <table class="w-full bg-white shadow-md rounded-lg overflow-hidden">
       <thead class="bg-gray-100">
           <tr>	
               ...
               <th class="px-4 py-2 text-left">email_verified_at</th>
               <th class="px-4 py-2 text-left">password</th>
               <th class="px-4 py-2 text-left">remember_token</th>
               <th class="px-4 py-2 text-left">created_at</th>
               <th class="px-4 py-2 text-left">updated_at</th>
       ...
       <tbody>
           @foreach($users as $user)
               <tr class="border-b hover:bg-gray-50">
                   ...
                   <td class="px-4 py-2">
                        <a href="{{ route('posts.editform', $user->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full mr-2">Update</a>
                        <form action="{{ route('posts.destroy', $user->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
